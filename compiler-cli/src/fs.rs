...
/// Use `git submodule add --name <NAME> -- <URL> <PATH>` to clone a particular Git package.
/// This is a workaround while Git dependencies don't exist yet.
/// If Git is not installed then we do nothing.
...
fn submodule_add_failed_error(name: &str, url: &str, path: &Utf8Path, error: String) {
    eprintln!(
        "WARNING: Failed to clone submodule '{name}' from URL '{url}' to path '{path}' \
due to the error below. You can try again by passing the appropriate values to the command \
\"git submodule add --name 'NAME HERE' -- 'URL HERE' 'PATH HERE'\". Alternatively, change \
the generated 'gleam.toml' file so it doesn't depend on those submodules. \
\nThe error was:\n{}",
        error
    );
}
