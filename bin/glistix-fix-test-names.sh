...
echo "${ANSI_BOLD}=== Replacing 'gleam_core/cli__' with 'glistix_core/cli__' in test snapshot names ===${ANSI_END}" >&2

curr_dir=""
files=$(find . -name 'gleam_core__*.snap' -o -name 'gleam_cli__*.snap' -o -name 'gleam__*.snap')
IFS=$'\n'
for i in ${files}
do
  # Replace the prefix
  newname="$(echo "$i" | sed -Ee 's;(^|/)gleam_core__;\1glistix_core__;' | sed -Ee 's;(^|/)gleam_cli__;\1glistix_cli__;' | sed -Ee 's;(^|/)gleam__;\1glistix__;')"
