    ...
    match req {
        ...
        Requirement::Git { git, .. }
            if !git.starts_with("https://")
                ...
                && !git.starts_with("ssh://")
                && !git.starts_with("git://")
                && (!git.contains('@') || !git.contains(':')) =>
        {
            // HACK: might be a local path, so we replace with something that
            // will surely be overridden by the root config instead of trying
            // to parse the path.
            Requirement::Hex {
                version: version::Range::new("=9998.999.9".to_string()),
            }
        }
        // Not a local path, so no need to override relative paths.
        _ => req.clone(),
    }
}
